...
class CND4(nn.Module):
    ...
    k2: float = 19.09
    k3: float = 254.56
    electronegativity_factor: bool = True
    ...
    def __call__(self, inputs):
        ...
        k0=self.k0
        if self.trainable:
            k0 = k0*jnp.abs(self.param("k0", lambda key : jnp.asarray(1.)))

        CNij = 0.5 * (1 + jax.scipy.special.erf(k0 * (1 - rij / rcij))) * graph["switch"]
        ...
        if self.electronegativity_factor:
            ...
            k3 = self.k3
            if self.trainable:
                k1 = k1*jnp.abs(self.param("k1", lambda key : jnp.asarray(1.)))
                k2 = self.param("k2", lambda key : jnp.asarray(1.))
                k3 = jnp.abs(self.param("k3", lambda key : jnp.asarray(1.)))
                en = self.param("en", lambda key : jnp.asarray(D3_ELECTRONEGATIVITIES))[species]
            else:
...
class CNShift(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        if self.kappa_key is not None:
            ...
            species = inputs["species"]
            kappai = self.param("kappa", nn.initializers.zeros, (len(D3_COV_RADII),))[species]
...
class CNStore(nn.Module):
    ...
    init_scale_cn: float = 5.
    init_scale_values: float = 1.
    beta: float = 6.
    trainable: bool = True
    output_dim: int = 1
    squeeze: bool = True
    ...
    def __call__(self, inputs):
        ...
        cn_refs = self.param("cn_refs", nn.initializers.uniform(self.init_scale_cn), (len(D3_COV_RADII), self.store_size))[species]
        values_refs = self.param("values_refs", nn.initializers.uniform(self.init_scale_values), (len(D3_COV_RADII), self.store_size,self.output_dim))[species]
        ...
        delta_cns = jnp.log(jnp.sum(jnp.exp(-beta*j*((cn[:,None]-cn_refs)**2)[:,:,None]),axis=-1))
